...
static uint16_t LIGHTCONTROL_MainBrightness;
static lightcontrol_feature_state_t LIGHTCONTROL_MainLightState;
...
void LIGHTCONTROL_Init(void) {
    ...
    LIGHTCONTROL_MainBrightness = LIGHTCONTROL_BRIGHTNESS_MIN;
    LIGHTCONTROL_MainLightState = lightcontrol_feature_state_ok;
...
uint16_t LIGHTCONTROL_GetCombinedBrightness() {
    return LIGHTCONTROL_MainBrightness;
}
...
void LIGHTCONTROL_SetBrightness(lightcontrol_segment_t segment, uint16_t brightness) {
    ...
    if (segment == lightcontrol_segment_main) {
        LIGHTCONTROL_MainBrightness = brightness;
    }
...
lightcontrol_feature_state_t LIGHTCONTROL_GetDiagnosticState(lightcontrol_segment_t segment) {
    if (segment == lightcontrol_segment_main) {
        return LIGHTCONTROL_MainLightState;
    }
