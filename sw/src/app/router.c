...
void ROUTER_Update10ms(void) {
    ...
    if (!ROUTER_DiagRequestActive) {
        uint16_t main_target = BRIGHTNESS_GetOutput(brightness_output_main);

        LIGHTCONTROL_SetBrightness(lightcontrol_segment_main, main_target);
    }
    else {
        ...
        for (int i = 0; i < ROUTER_MAX_DIAG_CHANNELS; i++) {
            if (ROUTER_DiagRequests[i].active) {
                ...
                if (ROUTER_DiagRequests[i].activeTime >= FEATURE_DIAG_DRIVER_CONTROL_TIMEOUT) {
                    ...
                    if (i == brightness_output_main) {
                        LIGHTCONTROL_SetBrightness(lightcontrol_segment_main, ROUTER_DiagRequests[i].brightness);
                    }
                }
            }
        }
...
bool ROUTER_DiagRequest(brightness_output_t output, uint8_t mode, uint16_t brightness)
{
    ...
    // TODO: disabled for now, will be re-enabled when speed data is available
    // if (
